	...
	<div class="main1">
		...
		<?php 
			if (isset($_POST['titre']) && isset( $_POST['titre'])) {
				try{
					...
					$req =$id_con->prepare("INSERT INTO subjects (title, date, time, id_user, id_theme) VALUES(?, ?, ?, ?, ?)");
					$req -> execute(array($titre, $jour, $time, $id_user, $id_theme));

					//$req =$id_con->query('SELECT id_subject FROM subjects WHERE title="'.$titre.'" ');
					//$var=$req->fetch();
					$req =$id_con->prepare('SELECT id_subject FROM subjects WHERE title= :titre');
					...
					$req =$id_con->prepare('INSERT INTO messages (title, text, date, time, id_user, id_subject) VALUES(?, ?, ?, ?, ?, ?)');
					$req -> execute(array($titre, $text, $jour, $time, $id_user, $var['id_subject']));
		...
		<?php 
			...
			try{
				...
				$req =$id_con->prepare('SELECT id_subject, id_theme, title, id_user, date, time FROM subjects WHERE id_theme= :theme ');
				$req -> execute(array('theme' => $_GET['theme']));
				...
				while ($resultat=$req->fetch()) {
					...
					$req1 = $id_con->prepare('SELECT id_user FROM users WHERE users.id_user= :id');
					$req1 -> execute(array('id' => $resultat['id_user']));
					$resultat1 = $req1->fetch();
					...
					$req2= $id_con->prepare('SELECT count(id_message) AS nb_message FROM messages WHERE id_subject= :id_subject');
					$req2 -> execute(array('id_subject' => $resultat['id_subject']));
					$resultat2=$req2->fetch();
					if ($resultat2==null) {
					...
					$req3 = $id_con->prepare('SELECT messages.date AS last_date, messages.time AS last_time FROM (subjects INNER JOIN messages ON subjects.id_subject = messages.id_subject) WHERE messages.id_subject = :id_subject ORDER BY messages.date desc, messages.time desc');
					$req3 -> execute(array('id_subject' => $resultat['id_subject']));
					$resultat3=$req3->fetch();
